...
exports.externalDictionaryService = void 0;
...
// ───────────────────────────────────────────────────────────
async function fetchRelatedWords(topic, limit = 25) {
...
// ───────────────────────────────────────────────────────────
exports.externalDictionaryService = {
    /**
     ...
     */
    generateForTopic: async (topic, count = 10, difficulty) => {
        ...
        let filtered = unique;
        if (difficulty === "easy")
            filtered = unique.filter((w) => w.length <= 6);
        else if (difficulty === "medium")
            filtered = unique.filter((w) => w.length >= 7 && w.length <= 9);
        else if (difficulty === "hard")
            filtered = unique.filter((w) => w.length >= 10);
        if (filtered.length === 0)
            filtered = unique; // fallback to unfiltered
        // Take top N
        ...
        for (let i = 0; i < top.length && results.length < safeCount; i += concurrency) {
            ...
            const entries = await Promise.all(batch.map((w) => fetchDictionaryEntry(w)));
...
//# sourceMappingURL=externalDictionary.service.js.map