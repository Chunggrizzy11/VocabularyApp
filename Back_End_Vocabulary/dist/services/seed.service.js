...
const wordBank_1 = require("../data/wordBank");
const SEED_TOPICS = [
...
exports.seedService = {
    ...
    seedTopic: async (def) => {
        ...
        let topic = await Topic_1.Topic.findOne({ title: def.title });
        if (!topic) {
            topic = await Topic_1.Topic.create({
                title: def.title,
        ...
        const existingCount = await Vocabulary_1.Vocabulary.countDocuments({ topicId: topic._id });
        let needed = (def.targetWordCount || 50) - existingCount;
        if (needed <= 0) {
            return { topic, wordsCreated: 0 };
        }
        // 3. Get matching words from offline bank
        const bankWords = wordBank_1.WORD_BANK.filter((w) => def.topicTags.includes(w.topic) && def.levels.includes(w.level));
        const existingWords = await Vocabulary_1.Vocabulary.find({ topicId: topic._id }).select("word");
        ...
        if (toInsert.length === 0) {
            const total = await Vocabulary_1.Vocabulary.countDocuments({ topicId: topic._id });
            await Topic_1.Topic.findByIdAndUpdate(topic._id, { totalWords: total });
            return { topic, wordsCreated: 0 };
        }
        // 4. Bulk insert
        ...
        await Vocabulary_1.Vocabulary.insertMany(docs);
        // 5. Update count
        const total = await Vocabulary_1.Vocabulary.countDocuments({ topicId: topic._id });
        await Topic_1.Topic.findByIdAndUpdate(topic._id, { totalWords: total });
    ...
    seedAll: async () => {
        ...
        for (const def of SEED_TOPICS) {
            const { topic, wordsCreated } = await exports.seedService.seedTopic(def);
            const totalWords = await Vocabulary_1.Vocabulary.countDocuments({ topicId: topic._id });
            results.push({ title: def.title, wordsCreated, totalWords });
        }
        return results;
    },
    /** Seed random words into an existing topic from word bank */
    seedIntoTopic: async (topicId, keyword, count) => {
        const topic = await Topic_1.Topic.findById(topicId);
        if (!topic)
            throw new Error(`Topic ${topicId} not found`);
        const bankWords = wordBank_1.WORD_BANK.filter((w) => w.topic === keyword || w.word.includes(keyword.toLowerCase()) || w.meaning.includes(keyword));
        const existing = await Vocabulary_1.Vocabulary.find({ topicId }).select("word");
        ...
        await Vocabulary_1.Vocabulary.insertMany(docs);
        const total = await Vocabulary_1.Vocabulary.countDocuments({ topicId });
        await Topic_1.Topic.findByIdAndUpdate(topicId, { totalWords: total });
        return docs.length;
    },
};
