...
exports.seedController = {
    ...
    getDefinitions: (_req, res) => {
        ...
        return (0, response_1.success)(res, defs);
    },
    ...
    seedTopic: async (req, res) => {
        try {
            ...
            if (!title)
                return (0, response_1.error)(res, "Missing required field: title", 400);
            const defs = seed_service_1.seedService.getDefinitions();
            const def = defs.find((d) => d.title === title);
            if (!def) {
                return (0, response_1.error)(res, `Unknown topic: "${title}". Use GET /api/seed/definitions to see available topics.`, 404);
            }
    ...
    seedIntoTopic: async (req, res) => {
        try {
            ...
            if (!keyword)
                return (0, response_1.error)(res, "Missing field: keyword", 400);
            const created = await seed_service_1.seedService.seedIntoTopic(id, keyword, count);
            return (0, response_1.success)(res, { created });
        }
        catch (e) {
            return (0, response_1.error)(res, e.message);
        }
